...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = pairs
    variableDomainsDict[GHOST0] = pairs
    variableDomainsDict[GHOST1] = pairs

    maxManhattanDistance = manhattanDistance((0, 0), (X_RANGE-1, Y_RANGE-1))
    maxObs = maxManhattanDistance + MAX_NOISE

    variableDomainsDict[OBS0] = range(maxObs + 1)
    variableDomainsDict[OBS1] = range(maxObs + 1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for i in eliminationOrder:
            factorList, joinedList = joinFactorsByVariable(
                factorList, i)
            if len(joinedList.unconditionedVariables()) >= 2:
                eliminatedList = eliminate(joinedList, i)
                factorList.append(eliminatedList)
        result = joinFactors(factorList)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, num in newPosDist.items():
                belief[newPos] = (self.beliefs[oldPos] * num) + belief[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            position = self.legalPositions[i % legalPositionsSize]
    ...
    def getBeliefDistribution(self):
        ...
        for i in self.particles:
            currentBelief[i] = (1.0/self.numParticles) + currentBelief[i]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.particles:
            weight = self.getObservationProb(
                observation, pacmanPosition, i, jailPosition)
            weightedParticles[i] += weight

        if weightedParticles.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weightedParticles.sample()
                              for i in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        newPosDist = dict()
        for i in self.particles:
            if i not in newPosDist:
                newPosDist[i] = self.getPositionDistribution(gameState, i)
            nextParticle.append(newPosDist[i].sample())

        self.particles = nextParticle
        "*** END YOUR CODE HERE ***"
